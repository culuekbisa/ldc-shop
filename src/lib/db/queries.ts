...
const MAINTENANCE_THROTTLE_MS = 6 * 60 * 60 * 1000;
const MAINTENANCE_SETTING_KEY = 'db_maintenance_at';
...
async function ensureDatabaseInitialized() {
    ...
    try {
        ...
        // Update schema version (lightweight init only)
    ...
    // Heavy maintenance moved to scheduled maintenance task
...
export async function runDatabaseMaintenance(options?: { force?: boolean }) {
    const force = options?.force === true;
    const now = Date.now();

    if (!force) {
        try {
            const last = await getSetting(MAINTENANCE_SETTING_KEY);
            const lastRun = Number(last || 0);
            if (now - lastRun < MAINTENANCE_THROTTLE_MS) return false;
        } catch {
            // best effort
        }
    }
    ...
    try {
        await setSetting(MAINTENANCE_SETTING_KEY, String(now));
    } catch {
        // best effort
    }
